...
Route::post('news/{id}/send_comment', 'NewsController@comment_send')
    ->name('news.comment_send');
Route::post('/loadmore/load_news', 'NewsController@load_news')
    ->name('loadmore.load_news');
/*Forum*/
Route::resource('forum', 'Forum\ForumController');
Route::group(['prefix' => 'forum'], function () {
    Route::post('{forum}/loadmore/load_forum_section',
        'Forum\ForumController@loadForumShow')->name('load.more.forum.show');
    Route::post('loadmore/load_forum_sections',
        'Forum\ForumController@loadForumIndex')
        ->name('load.more.forum.index');

    /*Topic*/
    Route::resource('topic', 'ForumTopic\TopicController');
    Route::group(['prefix' => 'topic'], function () {
        Route::post('{id}/comment', 'ForumTopic\TopicController@saveComments')
            ->name('topic.send_comment');

        /**set reputation like/dislike*/
        Route::get('{id}/get_rating', 'TopicRatingController@getRating')
            ->name('forum.topic.get_rating');
...
Route::group(['prefix' => 'replay'], function () {
    Route::post('loadmore/load_replay', 'Replay\ReplayController@loadReplay')
        ->name('load.more.replay');
    Route::get('{id}/download', 'Replay\ReplayHelper@download')
        ->name('replay.download');
    Route::post('{id}/download_count', 'Replay\ReplayHelper@downloadCount')
        ->name('replay.download.count');
    Route::post('{id}/send_comment', 'Replay\ReplayHelper@saveComments')
        ->name('replay.send_comment');

    /**set reputation like/dislike*/
    Route::get('{id}/get_rating', 'ReplayRatingController@getRating')
        ->name('replay.get_rating');
...
Route::post('tournament/loadmore/load_tournament',
...
Route::group(['middleware' => ['auth', 'ban', 'verified']], function () {
    ...
    Route::post('comment/{id}/set_rating',
        'CommentsRatingController@setRating')
...
Route::group([
    ...
    function () {
        ...
        /**reputation gallery*/
        Route::get('/user-gallery/{id}/get_rating',
            'UserGalleryRatingController@getRating')
            ->name('gallery.get_rating');
        ...
        Route::post('{id}/user-topics/load_sections', 'User\UserTopicsController@forumSectionsAjaxLoad')->name('user.topics.load.sections');
        /*** Ajax pagination user-sections-topics ***/
        Route::post('{id}/user-topics/load_sections_topics', 'User\UserTopicsController@forumSectionsTopicsAjaxLoad')->name('user.topics.load.sections.topics');
...
//        Route::post('{id}/user-comments/load_sections', 'User\UserCommentsController@forumSectionsAjaxLoad')->name('user.comments.load.sections');
        /*** Ajax pagination user-sections-topics ***/
        Route::post('{id}/user-comments/load_sections_comments', 'User\UserCommentsController@forumSectionsCommentsAjaxLoad')->name('user.comments.load.sections.comments');
        ...
        Route::get('{id}/edit', 'UserController@edit')
            ->name('edit_profile');
        Route::put('{id}/save', 'UserController@update')
            ->name('save_profile');
        ...
        Route::get('/message/{dialogue_id}/load',
            'UserMessagingController@loadMoreMessages')
            ->name('user.message_load');
...
Route::group(['prefix' => 'chat'], function () {
    ...
    Route::group(['middleware' => ['auth', 'ban', 'verified']],
        function () {
    ...
    Route::get('/get_messages', 'ChatController@get_messages')
        ->name('chat.get_messages');
    //    Route::post('/get_message', 'ChatController@get_message')->name('chat.get_message');

    Route::get('/get_externalsmiles', 'ChatController@get_externalsmiles')
        ->name('chat.get_smiles');
    Route::get('/get_externalimages', 'ChatController@get_externalimages')
        ->name('chat.get_images');
});

/*Galleries*/
Route::resource('galleries', 'Gallery\GalleriesController');
Route::post('galleries/loadmore/load_galleries',
    'Gallery\GalleriesController@loadGalleries')
    ->name('load.more.galleries');
Route::post('galleries/{id}/send_comment', 'User\GalleryHelper@saveComments')
    ->name('galleries.send.comment');

/*Search Replay*/
Route::get('replay-search', 'Replay\ReplaySearchController@index')
    ->name('replay.only.search');
Route::group(['prefix' => 'replay-search'], function () {
    Route::post('loadmore/load_search_replays_only',
        'Replay\ReplaySearchController@loadReplay')
        ->name('load.more.replay.only.search');
});
/*Search*/
Route::get('replay-news-search', 'Search\SearchController@index')
    ->name('search');
Route::group(['prefix' => 'replay-news-search'], function () {
    Route::group(['prefix' => 'loadmore'], function () {
        Route::post('load_search_news', 'Search\SearchController@loadNews')
            ->name('load.more.search.news');
        Route::post('load_search_replays',
            'Search\SearchController@loadReplay')
            ->name('load.more.search.replays');
    });
});
