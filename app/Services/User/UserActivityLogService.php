...
class UserActivityLogService
{
    ...
    public static function parametersForCreateImage($userGallery)
    {
        if ($userGallery->getTitle()) {
            $description = 'Изображение <a target="_blank" href="'
                .route('user-gallery.show',
                    [
    ...
    {
        return json_encode([
            'description' => 'Replay <a target="_blank" href="'
                .route('replay.show', ['replay' => $replay->id]).'">'
                .$replay->title.'</a>',
        ]);
    }
    ...
    {
        return json_encode([
            'description' => 'Пост <a target="_blank" href="'
                .route('topic.show', ['topic' => $forumTopic->id]).'">'
                .$forumTopic->title.'</a>',
        ]);
    }
