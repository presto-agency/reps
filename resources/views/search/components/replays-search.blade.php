...
<div class="gocu-replays border_shadow">
    @if($visible_title)
        <div class="gocu-replays__title">
            ...
            <p class="title__text night_text">{{__('Результаты поиска <'.request('search').'> в реплеях')}}</p>
        </div>
    @endif
    @if(isset($replay) && $replay->isNotEmpty())
            ...
            <div class="gocu-replays__match">
                <div class="match__author">
                    ...
                    <div class="subtitle__icons">
                        ...
                        <a class="items__download night_text night_text downloaded" data-id="{{$item->id}}"
        ...
        <div class="gocu-replays__button night_modal">
            <button type="button" class="button button__download-more night_text" id="load_more_replays_search_button"
                    data-id="{{ $last_id }}">
                {{__('Загрузить еще')}}
            </button>
        </div>
