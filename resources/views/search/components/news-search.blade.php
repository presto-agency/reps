...
<div class="breaking-news border_shadow">
    ...
    @if(isset($news) && $news->isNotEmpty())
                    ...
                    <div class="card-body__author">
                        @if(!empty($item->author))
                            @if(auth()->check() && auth()->user()->userViewAvatars())
                                <img src="{{ asset($item->author->avatarOrDefault()) }}" alt="avatar"
                            ...
                            @guest()
                                <img src="{{ asset($item->author->avatarOrDefault()) }}" alt="avatar"
                                     class="author__avatar img-fluid">
                            @endguest()
                            <p class="author__nickname">{{ $item->author->name }}</p>
                            <span
                                class="author__date">{{\Carbon\Carbon::parse($item->author->created_at)->format('H:i d.m.Y')}}</span>

                        @endif
                    </div>
                    <a href="{{ route('news.show', $item->id) }}">
                        <h2 class="card-body__title night_text">{{ clean($item->title) }}</h2>
                    </a>
                    <p class="card-body__text night_text">
                        {!! ParserToHTML::toHTML(clean($item->preview_content),'size') !!}
                    </p>
        ...
        <div class="breaking-news__button night_modal">
            <button type="button" class="button button__download-more" id="load_more_news_search_button"
