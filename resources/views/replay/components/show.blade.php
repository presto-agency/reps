    ...
    <div class="wrapper">
        <div class=" title_block">
            <div class="left_block">
                ...
                <span class="title_text night_text" title="{{ clean($replayShow->title) }}">
                          {{ clean($replayShow->title) }}
                    </span>
            </div>
            <div class="right_block">
                @if(!empty($replayShow->users))
                    <a href="{{route('user_profile',['id'=>$replayShow->users->id])}}"
                       title="{{$replayShow->users->name}}">{{$replayShow->users->name}}</a>
                    @auth()
        ...
        <div class="title_block_gray change_gray">
            <div class="title_top left_block">
                <p>
                    {!! ParserToHTML::toHTML($replayShow->content,'size')  !!}
                </p>
            </div>
            <div class="right_block">
                ...
                <span class="comment_count">{{$replayShow->comments_count}}</span>
                ...
                <span>{{$replayShow->created_at->format('H:i d.m.Y')}}</span>
            </div>
        </div>
        <div class="replay-content">
            <div class="col-xl-8 ">
                <div class="content_left">
                    ...
                    <div class="right_block">
                        @if(!empty($replayShow->firstCountries))
                ...
                <div class="content_left">
                    ...
                    <div class="right_block">
                        @if(!empty($replayShow->firstRaces))
                            <img class="icon_bars" alt="race" title="{{$replayShow->firstRaces->title}}"
                                 src="{{asset('images/default/game-races/' . $replayShow->firstRaces->title . '.png')}}"/>
                        @endif
                        <span class="night_text">{{__(' vs ')}}</span>
                        @if(!empty($replayShow->secondRaces))
                            <img class="icon_bars" alt="race" title="{{$replayShow->secondRaces->title}}"
                                 src="{{asset('images/default/game-races/' . $replayShow->secondRaces->title . '.png')}}"/>
                        @endif
                    </div>
                </div>
                <div class="content_left">
                    ...
                    <div class="right_block">
                        <span class="night_text">{{$replayShow->first_location}}</span>
                        <span class="night_text">{{__('vs')}}</span>
                        <span class="night_text">{{$replayShow->second_location}}</span>
                    </div>
                </div>
                <div class="content_left">
                    <div class="left_block">
                    ...
                    <div class="right_block">
                        <span class="night_text">{{$replayShow->rating}}</span>
                    </div>
                </div>
                @if(auth()->check() && auth()->user()->isNotUser())
                    <div class="replay-edit">
                        ...
                        <a href="{{route('user-replay.edit',['id' => $replayShow->user_id,'user_replay' =>$replayShow->id])}}">
            ...
            <div class="col-xl-4 content_right">
                @if(!empty($replayShow->maps))
                    @if(!empty($replayShow->maps->url) && checkFile::checkFileExists($replayShow->maps->url))
                        <p class="title">{{$replayShow->maps->name}}</p>
                        <img class="img-fluid" src="{{asset($replayShow->maps->url)}}" alt="map">
                    @else
                        <img class="img-fluid" src="{{asset($replayShow->maps->defaultMap())}}" alt="map">
                    @endif
                @else
                ...
                <div class="replay-rating">
                    <a href="#vote-modal" class="positive-vote vote-replay-up" data-toggle="modal" data-rating="1"
                       data-route="{{route('replay.set_rating',['id'=>$replayShow->id])}}">
                        ...
                        <span>{{$replayShow->positive_count}}</span>
                    </a>
