<section class="user_reputation user_profile">
    <div class="wrapper border_shadow">
        <div class="title_block tb_reputation">
            ...
            <p class="title_text" title="{{Auth::user()->name}}">Информация о пользователе {{Auth::user()->name}}</p>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-xl-8 col-8 container_information">
                    <div class="userText_block">
                        <span class="title night_text">{{Auth::user()->name}}</span>
                        @if(Auth::user()->isOnline())
                            <span class="date">online</span>
                        @else
                            <div class="date">{{\Carbon\Carbon::parse(Auth::user()->activity_at)->diffForHumans()}}</div>
                        @endif
                    </div>
                    <div class="information_block">
                        <div class="left_block"><span>Имя:</span></div>
                        <div class="right_block"><span class="night_text">{{Auth::user()->name}}</span></div>
                    </div>
                    <div class="information_block">
                        <div class="left_block"><span>Страна:</span></div>
                        <div class="right_block"><span class="night_text">{{Auth::user()->countries->name}}</span></div>
                    </div>
                    <div class="information_block">
                        <div class="left_block"><span>Репутация:</span></div>
                        <div class="right_block"><span class="blue">{{Auth::user()->countries->count_positive - Auth::user()->countries->count_negative}} кг</span>
                        </div>
                    </div>
                </div>
            </div>
