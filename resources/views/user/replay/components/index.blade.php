...
<div class="gocu-replays border_shadow">
    ...
    @endif
    @if(isset($userReplaysAjaxLoad) && $userReplaysAjaxLoad->isNotEmpty())
        @foreach($userReplaysAjaxLoad as $item)
            <div class="gocu-replays__subtitle change_gray">
                ...
                <div class="gocu-replays__button night_modal litle_button">
                    <a href="{{route('replay.get_rating',['id' => $item->id])}}"
                       class="button button__download-more night_text">{{__('Рейтинг лист')}}</a>
                </div>
            ...
            <div class="gocu-replays__match">
                ...
                <p class="match__comment night_text">{!! ParserToHTML::toHTML(clean($item->content),'size')!!}</p>
        ...
        <div class="gocu-replays__button night_modal">
            <button type="button" class="button button__download-more night_text" id="load_more-user-replay"
                    data-id="{{ $last_id }}">
                {{__('Загрузить еще')}}
            </button>
        </div>
