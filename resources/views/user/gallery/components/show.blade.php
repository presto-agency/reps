    ...
    @if(!empty($image))
        <div class="gallery-detail__body night_modal border_shadow">
            <div class="body__items">
                <div class="items__title">{{ clean($image->sign) }}</div>
                <div class="items__rating">
                    @php
                        $modal = (!Auth::guest() && $image->user_id == Auth::user()->id) ?'#no-rating':'#vote-modal';
                    @endphp
                    <a href="#vote-modal" class="rating__like positive-vote vote-replay-up night_text"
                       data-toggle="modal"
                       data-rating="1"
                       data-route="{{route('gallery.set_rating',['id'=>$image->id])}}">
                        ...
                        <span class="night_text">{{$image->positive_count}}</span>
                    </a>
                    <a href="#vote-modal" class="rating__dislike negative-vote vote-replay-down night_text"
                       data-toggle="modal"
                       data-rating="-1"
                       data-route="{{route('gallery.set_rating',['id'=>$image->id])}}">
                        ...
                        <span class="night_text">{{$image->negative_count}}</span>
                    </a>
                </div>
                <div class="items__reputation-button">
                    <a class="night_text"
                       href="{{route('gallery.get_rating',['id' => $image->id])}}">{{__('Рейтинг лист')}}</a>
                </div>
                @if(request()->route()->getName() == 'user-gallery.show')
                    <div class="items__slide-button">
                        @if(!empty($previous))
                            <a href="{{route('user-gallery.show',['id'=> $image->user_id,'user_gallery'=> $previous])}}">
                                <i class="fas fa-angle-double-left"></i>
                            </a>
                        @endif
                        @if(!empty($next))
                            <a href="{{route('user-gallery.show',['id'=> $image->user_id,'user_gallery'=> $next])}}">
